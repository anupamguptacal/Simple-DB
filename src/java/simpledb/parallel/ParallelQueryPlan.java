...
public class ParallelQueryPlan {
    ...
    private OpIterator extractSlaveWorkerPlan(OpIterator root) {
    ...
    public static ParallelQueryPlan parallelizeQueryPlan(TransactionId tId,
                                                         OpIterator localQueryPlan, SocketInfo[] workers,
        ...
        if (slavePlanWrapper.collectorOperator != null) {// need a master worker
            ...
            OpIterator tmp = slavePlanWrapper.queryPlan;
    ...
    protected static class QueryPlanWrapper {
        OpIterator queryPlan;
        OpIterator collectorOperator;

        QueryPlanWrapper(OpIterator queryPlan, OpIterator collector) {
            this.queryPlan = queryPlan;
            this.collectorOperator = collector;
        }

        QueryPlanWrapper(OpIterator queryPlan) {
            this.queryPlan = queryPlan;
            this.collectorOperator = null;
        }
    }

    protected static OpIterator buildMasterWorkerPlan(TransactionId tId,
                                                      OpIterator root, QueryPlanWrapper slavePlanWrapper,
                                                      SocketInfo[] sourceWorkers) {
        if (slavePlanWrapper.collectorOperator == null)
            return slavePlanWrapper.queryPlan;

        OpIterator[] children = new OpIterator[] {};
        ...
        if (root == slavePlanWrapper.collectorOperator) {
            if (root instanceof Aggregate || root instanceof OrderBy) {
                ...
                ((Operator) root).setChildren(new OpIterator[] { c });
                return root;
            }
    ...
    protected static QueryPlanWrapper buildSlaveWorkerPlan(OpIterator root,
        ...
        if (root instanceof HashEquiJoin || root instanceof Join) {
            ...
            ((Operator) root).setChildren(new OpIterator[] { s11, s21 });
            ...
            if (child.collectorOperator != null)
                ...
                if (a.groupField() == Aggregator.NO_GROUPING) {
                    ...
                    ((Aggregate) root).setChildren(new OpIterator[] { sc });
            ...
            if (child.collectorOperator != null)
                ...
                f.setChildren(new OpIterator[] { child.queryPlan });
                child.queryPlan = f;
                return child;
            }
